...
const QUEUE_MODE = (__ENV.QUEUE_MODE || 'shared').toLowerCase();
const USE_SIMPLE = __ENV.USE_SIMPLE === '1';

const SEED_TOKENS = __ENV.SEED_TOKENS !== '0';
const TOKENS_PER_USER = Number.parseInt(__ENV.TOKENS_PER_USER || '5', 10);

const AUTO_TOP_UP = __ENV.AUTO_TOP_UP !== '0';
const TOP_UP_TOKENS = Number.parseInt(__ENV.TOP_UP_TOKENS || '5', 10);
const BALANCE_CHECK_EVERY = Number.parseInt(__ENV.BALANCE_CHECK_EVERY || '3', 10);

const THINK_TIME_MIN = Number.parseFloat(__ENV.THINK_TIME_MIN || '0.5');
const THINK_TIME_MAX = Number.parseFloat(__ENV.THINK_TIME_MAX || '1.5');
const SESSION_HOLD_SECONDS = Number.parseInt(
  __ENV.SESSION_HOLD_SECONDS || '45',
  10,
);

const SESSION_VUS = Number.parseInt(__ENV.SESSION_VUS || '2000', 10);
...
export function setup() {
  ...
  if ((SEED_TOKENS || AUTO_TOP_UP) && !STRIPE_WEBHOOK_SECRET) {
...
export default function (data) {
  ...
  if (AUTO_TOP_UP && __ITER % BALANCE_CHECK_EVERY === 0) {
    ensureTokenBalance(baseUrl, ctx);
  }

  const joinRes = http.post(
    `${baseUrl}/queue/join`,
    JSON.stringify({ region: QUEUE_REGION, preferences: buildPreferences() }),
    {
...
function buildOptions() {
  ...
  const stages = [
    { duration: RAMP_UP, target: SESSION_VUS },
    { duration: HOLD, target: SESSION_VUS },
...
function buildPreferences() {
  if (QUEUE_MODE === 'isolated') {
    return { seed: `${__VU}-${__ITER}-${Math.floor(Math.random() * 1e9)}` };
  }
  return { mode: 'video-session-sustain' };
}
...
function ensureTokenBalance(baseUrl, ctx) {
  const balanceRes = http.get(`${baseUrl}/tokens/balance`, {
    headers: authHeaders(ctx.accessToken),
    tags: { name: 'tokens_balance' },
  });

  const ok = check(balanceRes, {
    'tokens balance ok': (r) => r.status === 200,
  });
  errorRate.add(!ok);
  ...
  const tokenBalance = balanceRes.json()?.tokenBalance ?? 0;
  if (tokenBalance < 1) {
    seedTokens(baseUrl, ctx.userId, TOP_UP_TOKENS);
  }
}
