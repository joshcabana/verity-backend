...
describe('Settings', () => {
  ...
  it('lists blocked users and unblocks one', async () => {
    apiJsonMock.mockImplementation(async (path: string, options?: { method?: string }) => {
      if (path === '/moderation/blocks' && (!options || options.method === undefined)) {
        return {
          ok: true,
          status: 200,
          data: [
      ...
      if (path === '/moderation/blocks/user-2' && options?.method === 'DELETE') {
        return {
          ok: true,
          status: 200,
    ...
    renderWithProviders(<Settings />, { route: '/settings', path: '/settings' });
