...
docker run --rm --network "$NETWORK_NAME" \
  ...
  bash -lc "mkdir -p /tmp/app \
    ...
    && DATABASE_URL='postgres://postgres:postgres@${PG_CONTAINER}:5432/verity' REDIS_URL='redis://${REDIS_CONTAINER}:6379' JWT_ACCESS_SECRET='test_access_secret' JWT_REFRESH_SECRET='test_refresh_secret' JWT_SECRET='test_shared_secret' npm run test:e2e -- queue-session-decision.e2e-spec.ts"
